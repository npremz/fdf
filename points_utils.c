...
/*   Updated: 2023/11/20 16:50:20 by npremont         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	*ft_get_rpoint(double *point, t_dim plan)
{
	while (plan.points)
	{
		if (((double *)plan.points->content)[0] == point[0] + STANCE &&
...
double	*ft_get_bpoint(double *point, t_dim plan)
{
	while (plan.points)
	{
		if (((double *)plan.points->content)[1] == point[1] + STANCE &&
